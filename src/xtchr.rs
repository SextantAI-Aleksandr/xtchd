...
impl Xtchr {
    ...
    pub async fn article_detail(&self, art_id: i32) -> Result<views::ArticleDetail, DiskError> {
        ...
        p.art_paras
        FROM articles ar LEFT JOIN authors au ON ar.auth_id = au.auth_id
        ...
        let author: XtchdContent<xrows::Author> = row.get(1);
        let article: XtchdContent<xrows::Article> = row.get(2);
        let paragraphs: Vec<XtchdContent<xrows::ArticlePara>> = row.get(3);
